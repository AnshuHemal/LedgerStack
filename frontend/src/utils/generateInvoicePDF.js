...
export const generateInvoicePDF = async (invoiceData) => {
  ...
  try {
    // Fetch the HTML template (prefer invoiceTemp.html, fallback to invoiceTemplate.html)
    let htmlTemplate;
    try {
      let response = await fetch("/invoiceTemp.html");
      if (response.ok) {
    ...
    const pdfOptions = {
      ...
      filename: `Invoice_${invoiceData.bill_no?.bill_prefix || "Invoice_"}${
        invoiceData.bill_no?.no || "N/A"
      }_${new Date(invoiceData.bill_date).toISOString().split("T")[0]}.pdf`,
...
const populateInvoiceData = (container, invoiceData) => {
  ...
  setElementText(
    container,
    "#invoice-number",
    `${invoiceData.bill_no?.bill_prefix || ""}${
      invoiceData.bill_no?.no || "N/A"
    }`
  );
  ...
  setElementText(
    container,
    "#transport",
    invoiceData.transportation_account?.name || "N/A"
  );
  ...
  setElementText(
    container,
    "#receiver-name",
    invoiceData.delivery_party_account?.name || "N/A"
  );
  setElementText(
    container,
    "#receiver-address",
    formatAddress(invoiceData.delivery_party_account)
  );
  setElementText(
    container,
    "#receiver-contact",
    formatContact(invoiceData.delivery_party_account)
  );
  setElementText(
    container,
    "#receiver-state",
    invoiceData.delivery_party_account?.state || "N/A"
  );
  setElementText(
    container,
    "#receiver-state-code",
    getStateCode(invoiceData.delivery_party_account?.state) || "N/A"
  );
  setElementText(
    container,
    "#receiver-gst",
    invoiceData.delivery_party_account?.gst || "N/A"
  );
  setElementText(
    container,
    "#receiver-pan",
    invoiceData.delivery_party_account?.panNo || "N/A"
  );
  ...
  // Products table (only populate product details for now)
  populateProductsTable(container, invoiceData.products);

  // Calculate totals and taxes
  const totalBoxes = invoiceData.products.reduce(
    (acc, prod) => acc + Number(prod.boxes || 0),
    0
  );
  const totalQuantity = invoiceData.products.reduce(
...
const populateProductsTable = (container, products) => {
  // Prefer the items table used in invoiceTemp.html; fall back to products-tbody (older template)
  const tbody =
    container.querySelector(".items-table tbody") ||
    container.querySelector("#products-tbody");
  if (!tbody) return;
  ...
  products.forEach((product, index) => {
    ...
    const noOfPcs = Number(product.no_of_pcs || 0);
    const quantity = boxes * noOfPcs;
    const rate = Number(product.rate || 0);
    const discount = Number(product.discount || 0);
    const igst =
      product.igst !== null && product.igst !== undefined
        ? Number(product.igst)
        : 0;
    const cgst =
      product.cgst !== null && product.cgst !== undefined
        ? Number(product.cgst)
        : 0;
    const sgst =
      product.sgst !== null && product.sgst !== undefined
        ? Number(product.sgst)
        : 0;
    ...
    const finalAmount =
      baseAmount -
      discountAmt +
      ((baseAmount - discountAmt) * gstPercentage) / 100;
    ...
    row.innerHTML = `
      ...
      <td class="num">${igst || 0}%</td>
      <td class="num">${cgst || 0}%</td>
      <td class="num">${sgst || 0}%</td>
      <td class="num">${finalAmount.toFixed(2)}</td>
    `;
