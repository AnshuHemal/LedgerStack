...
const ProformaInvoiceOverview = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setForm((prev) => ({
          ...prev,
          bill_no: { ...prev.bill_no, no: currentBillNo },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        `${INCOME_EXPENSES_URL}/proforma-invoice`,
        form,
        {
          withCredentials: true,
        }
      );
      await axios.put(`${INCOME_EXPENSES_URL}/proforma-bill`, {
        no: updatedBillNo,
      });
  ...
  const renderContent = () => {
    if (selectedTab === "proforma-display") {
  ...
  return (
    <>
      <div className="child__container d-flex justify-content-between align-items-start">
        ...
        <div className="me-3 d-flex text-center align-items-center gap-2">
          <button
            ...
            onClick={() => setShowProformaAddModal(true)}
          >
            + Proforma
          </button>
        </div>
      ...
      <div className="nav-tabs d-flex gap-3">
        <button
          className={`nav-tab ${
            selectedTab === "proforma-display" ? "active" : ""
          }`}
          onClick={() => {
            handleTabClick("proforma-display");
          }}
        >
          Proforma Display
        </button>
      ...
      {/* Modal for adding Proforma Invoice */}
      <div
        className={`modal fade ${showProformaAddModal ? "show" : ""}`}
        style={{ display: showProformaAddModal ? "block" : "none" }}
        tabIndex="-1"
...
export default ProformaInvoiceOverview;
