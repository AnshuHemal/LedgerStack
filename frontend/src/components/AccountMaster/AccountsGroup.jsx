...
const AccountsGroup = () => {
  ...
  const [formData, setFormData] = useState({ name: "", effect: "" });
  ...
  const handleRowClick = async (groupId) => {
    try {
      const response = await axios.get(`${API_URL}/account-group/${groupId}`);
      setFormData({
  ...
  const handleEdit = async () => {
    try {
      const response = await axios.put(
        `${API_URL}/account-group/${selectedGroupId}`,
        formData
      );
      toast.success(response.data.message);
      setShowModal(false);
    } catch (err) {
      toast.error("Error updating account group");
    }
  };
